...
public class InformeControllerWebTestClientIT extends AbstractIntegration {
    ...
    private void subirInforme() {
        String prediccion = "Not cancer";
        String contenido = "La imagen no muestra signos de cáncer.";
    ...
    @Test
    @DisplayName("Crear informe y verificar que se ha creado correctamente")
    void createInformePost_isObtainedWithGet() {
        subirInforme();

        testClient.get().uri("/informe/{id}", informeId)
                .exchange()
                .expectStatus().isOk()
                .expectBody(Informe.class)
                .value(informe -> {
                    assertTrue(informe.getPrediccion().contains("cancer"));
                    assertTrue(informe.getContenido().equals("La imagen no muestra signos de cáncer."));
                    assertTrue(informe.getImagen().equals(imagen));
                });
    }
}
